<script>
	...
	let signup = $state({ name: '', email: '', phone: '', card: '', building: '', password: '' });
	...
	async function createAccount() {
		if (!signup.name || !signup.email || !signup.password || !signup.building) {
			message = 'Please add your name, email, password, and building.';
			return;
		}
		loading = true;
		message = '';
		try {
			const response = await fetch(`${API_URL}/users`, {
				...
				body: JSON.stringify({ building_id: Number(signup.building), name: signup.name, email: signup.email, password: signup.password, role: 'tenant' })
			});
			const data = await response.json();
			...
			signup = { name: '', email: '', phone: '', card: '', building: '', password: '' };
			password = '';
	...
	async function loadBuildings() {
		try {
			...
			const data = await response.json();
			if (response.ok) buildingOptions = data;
		} catch (error) { /* signup form stays usable even if this fails */ }
	}
	...
	async function toggleMachine(machine) {
		const next = machine.status === 'Maintenance' ? 'active' : 'maintenance';
		loading = true;
		try {
			...
			const data = await response.json();
			if (!response.ok) throw new Error(data.error || 'Could not update the machine.');
			await loadDashboard();
			message = `${machine.name} is now ${next === 'active' ? 'available' : 'in maintenance'}.`;
		} catch (error) { message = error.message; }
		finally { loading = false; }
	}
	...
	async function recordBookingPayment(bookingId, amount) {
		...
		} catch (error) { /* the booking still succeeded even if the payment record fails */ }
	}
